...
import { AlertCircle, ArrowLeft, Check, Clapperboard, Image as ImageIcon, Sparkles, Upload, WandSparkles, Loader2, ChevronRight } from "lucide-react";
...
export default function CreatePage() {
  ...
  async function refreshUsage() {
    try {
      ...
      setImagesUsed(Number(data.imagesUsed || 0));
      setVideosUsed(Number(data.videosUsed || 0));
    } catch {}
  }
  ...
  function enhancePrompt() { if (!prompt.trim()) return; setPrompt(`${prompt.trim()}, cinematic composition, professional lighting, highly detailed, polished visual quality`.slice(0, MAX_PROMPT_LENGTH)); setEnhanced(true); setInfo("Prompt enhanced"); }
  ...
  async function generate() {
    ...
    try {
      ...
      if(!response.ok){
        ...
        else if (data.code === "DAILY_LIMIT_REACHED") setError(data.error || `Daily ${mode} limit reached. Try again tomorrow.`);
      ...
      if (mode === "image") {
        setImagesUsed(v => Math.min(10, v + 1));
      ...
      if(generationId){
        for(let attempt=0;attempt<72;attempt++){
          ...
          if(!poll.ok&&result.error){setError(result.error);await refreshUsage();return;}
          const next=Math.max(5,Math.min(99,Math.round(Number(result.progress||0))));setProgress(next);
          if(result.status==="completed"){setProgress(100);await refreshUsage();setSuccess("Video completed and saved to your private gallery.");window.setTimeout(()=>window.location.href="/creations",500);return;}
          if(result.status==="failed"){setError(result.error||"Video generation failed.");await refreshUsage();return;}
      ...
      setError("Video request was accepted but no generation ID was returned.");
      await refreshUsage();
