...
export async function POST(request: Request) {
  ...
  const { error: userSaveError } = await supabase.from("createx_chat_messages").insert({ chat_id: chatId, user_id: user.id, role: "user", content: userText, file_name: file instanceof File ? file.name : null });
