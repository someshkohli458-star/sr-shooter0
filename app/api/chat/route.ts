...
async function zipToText(file: File) {
  ...
  for (const name of Object.keys(zip.files).slice(0, 120)) {
    ...
    if (isTextLike(new File([], name))) {
      try { entries.push(`FILE: ${name}\n${(await entry.async("string")).slice(0, 12000)}`); } catch {}
    } else entries.push(`FILE: ${name} [binary or unsupported text]`);
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) history = parsed.filter(m => (m?.role === "user" || m?.role === "assistant") && typeof m?.content === "string").slice(-20).map(m => ({ role: m.role, content: m.content.slice(0, 12000) }));
  } catch {}
  ...
  if (file instanceof File && file.size > 0) {
    ...
    if (mime.startsWith("image/")) {
      ...
      const extracted = await zipToText(file);
      content.push({ type: "input_text", text: `ZIP project: ${file.name}\n${extracted || "No readable source files found."}` });
    } else if (isTextLike(file)) {
      content.push({ type: "input_text", text: `Attached source/file: ${file.name}\n\n${(await file.text()).slice(0, MAX_CONTEXT_CHARS)}` });
    } else {
