...
export async function POST(request: Request) {
  ...
  try {
    if (type === "video") {
      ...
      const nextSettings = { ...settings, provider: "openai", provider_id: result.id, provider_status: result.status || "queued", daily_usage: usage };
      ...
      return NextResponse.json({ generation: updated, provider: "openai-sora-2", providerId: result.id, usage }, { status: 202 });
    }
