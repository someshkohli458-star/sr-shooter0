...
export async function GET(request: Request) {
  ...
  const providerId = String((generation.settings as Record<string, unknown>)?.provider_id || "");
  ...
  if (provider.status === "failed") {
    const cost = Number((generation.settings as Record<string, unknown>)?.cost || 0);
    if (cost > 0) await supabase.rpc("refund_createx_credit", { p_user_id: user.id, p_amount: cost });
    await supabase.from("createx_generations").update({ status: "failed", settings: { ...(generation.settings as Record<string, unknown>), provider_status: "failed", provider_error: provider.error?.message || "Video generation failed" }, updated_at: new Date().toISOString() }).eq("id", id).eq("user_id", user.id);
  ...
  if (provider.status !== "completed") {
    await supabase.from("createx_generations").update({ settings: { ...(generation.settings as Record<string, unknown>), provider_status: provider.status, progress: provider.progress || 0 }, updated_at: new Date().toISOString() }).eq("id", id).eq("user_id", user.id);
  ...
  const settings = { ...(generation.settings as Record<string, unknown>), provider_status: "completed", progress: 100 };
  const { error: updateError } = await supabase.from("createx_generations").update({ status: "completed", result_path: storagePath, result_url: null, settings, updated_at: new Date().toISOString() }).eq("id", id).eq("user_id", user.id);
