...
async function refundDaily(supabase: Awaited<ReturnType<typeof createClient>>, userId: string) {
  await supabase.rpc("refund_createx_daily_generation", { p_user_id: userId, p_type: "video" });
}
...
export async function GET(request: Request) {
  ...
  if (generation.status === "failed") return NextResponse.json({ status: "failed", progress: 0 });
  ...
  const response = await fetch(`https://api.openai.com/v1/videos/${encodeURIComponent(providerId)}`, {
  ...
  if (provider.status === "failed") {
    await refundDaily(supabase, user.id);
    await supabase.from("createx_generations").update({
      status: "failed",
      settings: { ...settings, provider_status: "failed", provider_error: provider.error?.message || "Video generation failed" },
  ...
  if (provider.status !== "completed") {
    await supabase.from("createx_generations").update({
      settings: { ...settings, provider_status: provider.status, progress: provider.progress || 0 },
      updated_at: new Date().toISOString(),
    }).eq("id", id).eq("user_id", user.id);
    return NextResponse.json({ status: provider.status, progress: provider.progress || 0 });
  }

  const content = await fetch(`https://api.openai.com/v1/videos/${encodeURIComponent(providerId)}/content`, {
    headers: { Authorization: `Bearer ${apiKey}` },
    cache: "no-store",
  });
  ...
  const { error: updateError } = await supabase.from("createx_generations").update({
    status: "completed",
