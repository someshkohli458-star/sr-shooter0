...
export default function PricingPage() {
  ...
  async function buy(packCode: string) {
    ...
    try {
      const res = await fetch("/api/payments/create-order", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ packCode }) });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { setMessage(data.error || "Please sign in to buy credits."); return; }
      if (!window.Razorpay) { setMessage("Payment checkout is still loading. Please try again."); return; }
      const checkout = new window.Razorpay({ key: data.keyId, amount: data.amount, currency: data.currency, name: "CreateX AI", description: `${data.credits} AI credits`, order_id: data.providerOrderId, prefill: {}, theme: { color: "#8b5cf6" }, handler: async (response: any) => {
        const verify = await fetch("/api/payments/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId: data.orderId, ...response }) });
        const result = await verify.json().catch(() => ({}));
        if (!verify.ok) setMessage(result.error || "Payment verification failed.");
        else setMessage(`Payment successful. ${result.credits} credits are now available.`);
  ...
  return <main className="min-h-screen bg-[#07070a] text-white">
    ...
    <section className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24"><div className="mx-auto max-w-2xl text-center"><p className="text-sm font-semibold text-violet-300">Credits</p><h1 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">Create more. Pay only when you create.</h1><p className="mt-5 text-white/45">Simple credit packs for AI images and videos. One-time purchase.</p></div>
      ...
      <div className="mx-auto mt-10 max-w-3xl rounded-3xl border border-white/10 bg-white/[.025] p-6 text-center"><p className="text-sm text-white/45">Secure checkout is prepared for Razorpay. Add Razorpay keys in Vercel Environment Variables before going live.</p><Link href="/create" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-violet-300">Start creating <ArrowRight size={15}/></Link></div>
    </section>
  </main>;
}
