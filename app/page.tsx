...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#050507] text-white selection:bg-violet-400/30">
      ...
      <section id="gallery" className="relative z-10 mx-auto max-w-7xl px-5 pb-24 md:px-8"><div className="flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.25em] text-cyan-300">Explore</p><h2 className="mt-2 text-3xl font-black tracking-tight md:text-5xl">Made with CreateX.</h2></div><span className="hidden text-xs text-white/25 sm:block">A glimpse of what&apos;s possible</span></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{samples.map(([title,sub,image],i)=><div key={title} className="group overflow-hidden rounded-[28px] border border-white/10 bg-white/[.025] transition hover:-translate-y-1 hover:border-white/20"><div className="relative aspect-[4/5] overflow-hidden bg-black"><img src={image} alt={title} loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-white/5"/><div className="absolute bottom-4 left-4 rounded-full border border-white/15 bg-black/35 px-2.5 py-1 text-[9px] text-white/65 backdrop-blur">0{i+1}</div></div><div className="p-4"><div className="text-sm font-bold">{title}</div><div className="mt-1 text-[10px] text-white/30">{sub}</div></div></div>)}</div></section>
